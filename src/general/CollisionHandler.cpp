 ...
 */
bool linesIntersect(float q1x, float x2, float q1y, float y2, float q2x, float x4, float q2y, float y4, float margin){
	...
	return s >= 0 - margin && s <= 1 + margin && t >= 0 - margin && t <= 1 + margin; //s and t are only defined in [0;1], so only if they are in this range there is a solution
}
 ...
 * 			x / y = endpoint of line
 */
float* getLineIntersection(float q1x, float x2, float q1y, float y2, float q2x, float x4, float q2y, float y4, float margin){
	float *f = new float[2];
	f[0] = f[1] = nanf("");
	float v1x = x2 - q1x, v1y = y2 - q1y, v2x = x4 - q2x, v2y = y4 - q2y; //define direction vectors with length of line segment
	double s = (- v1x * (q2y - q1y) + v1y * (q2x - q1x)) / (v1x * v2y - v1y * v2x); //calculate intersection point variable for first line segment
	double t = (- v2x * (q1y - q2y) + v2y * (q1x - q2x)) / (v2x * v1y - v2y * v1x); //calculate intersection point variable for second line segment
	if(s >= 0 - margin && s <= 1 + margin && t >= 0 - margin && t <= 1 + margin){ //s and t are only defined in [0;1], so only if they are in this range there is a solution
		f[0] = q1x + t * v1x;
		f[1] = q1y + t * v1y;
		return f;
	}
