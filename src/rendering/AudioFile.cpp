	...
	namespace audio{
		...
		void AudioFile::open(){
			...
			if(format == WAV){
				...
				//		warpLogger.log("opening audio file: " + filePath);
				...
				if(std::string(buffer) != "RIFF"){
					warpLogger.log(filePath + " is no sound file! Instead found tag: " + std::string(buffer));
					return;
				}
				...
				if(std::string(buffer) != "WAVE"){
					warpLogger.log(filePath + " is no wave file!");
					return;
				}

				file.read(buffer, 4); //read WAVE
				if(std::string(buffer) != "fmt "){
					warpLogger.log(filePath + " has no format tag.");
					return;
				}
				...
				if(blockSize != 16){
					warpLogger.log(filePath + " fmt block size is not 16. unknown format (maybe compressed audio?).");
					return;
				}
				file.read(buffer, blockSize);
				if(buffer[0] != 1){
					warpLogger.log(filePath + " unknown wave format.");
					return;
				}
				uint16_t u16 = 0;
				uint32_t u32 = 0;
				...
				//		warpLogger.log("channel count: " + std::to_string(channelCount));
				//		warpLogger.log("bit count: " + std::to_string(bitCount));
				//		warpLogger.log("samplerate: " + std::to_string(sampleRate));
				...
				if(std::string(buffer) != "data"){
					warpLogger.log(filePath + " has no data tag. Instead: " + std::string(buffer));
					return;
				}
				u32 = 0;
				file.read(buffer, 4);
				...
				if(cfile != NULL){
					...
					if((error = ov_open_callbacks(cfile, ogg, NULL, 0, mode == STREAM ? OV_CALLBACKS_STREAMONLY : OV_CALLBACKS_DEFAULT)) < 0){
						switch(error){
							case OV_EREAD:
								warpLogger.log("ERROR: A read from media returned an error while reading " + filePath);
								break;
							case OV_ENOTVORBIS:
								warpLogger.log("ERROR: Bitstream for " + filePath + " does not contain any Vorbis data.");
								break;
							case OV_EVERSION:
								warpLogger.log("ERROR: Vorbis version mismatch. File: " + filePath);
								break;
							case OV_EBADHEADER:
								warpLogger.log("ERROR: Invalid Vorbis bitstream header. File: " + filePath);
								break;
							case OV_EFAULT:
								warpLogger.log("ERROR: Internal logic fault; indicates a bug or heap/stack corruption. File: " + filePath);
								break;

						}
						//warpLogger.log("ERROR: unable to open file " + filePath + " as ogg");
						fclose(cfile);
					}
					else{
						...
						if(i != NULL){
							...
							warpLogger.log("ERROR: unable to retrieve ogg file info");
						}
					}
				}
				else{
					warpLogger.log("ERROR: Unable to open Audio File");
				}
		...
		void AudioFile::printFileInfo(){
			warpLogger.log("Audio File: \t" + filePath);
			warpLogger.log("Sameplerate: \t" + std::to_string(sampleRate));
			warpLogger.log("Bitcount: \t\t" + std::to_string(bitCount));
			warpLogger.log("Channel count: \t" + std::to_string(channelCount));
			warpLogger.log("Data size: \t" + std::to_string(dataSize));
		}
