...
void checkForAlError(std::string pos = "unknown"){
	...
	if(error != AL_NO_ERROR){
		...
		warpLogger.log("ERROR: audio at " + pos + ": " + errormsg);
	}
}

void init(){
#ifdef AUDIO
	warpLogger.log("initializing audio...");
	...
	if(device){
		...
		warpLogger.log("ERROR: Unable to create OpenAL device");
		return;
	}
	alGetError();
	...
	int error = alGetError();
	if(error != AL_NO_ERROR){
		warpLogger.log("ERROR while generating sources: " + error);
	}
	for(int i : sources){
...
//			f.printFileInfo();
			...
			warpLogger.log("There already is a buffer named: " + name);
		}
	}
	else{
...
int playSound(std::string name, float x, float y, float volume, float pitch, bool looping){
	if(audioInitialized){
		if(buffers.find(name) != buffers.end()){
			warpLogger.log("playing sound: " + name);
			...
			warpLogger.log("ERROR: no sound named " + name);
		}
	}
	else{
	...
	if(currentMusicFile == NULL){
		if(musicQueue.size() > 0){
			...
			if(state != AL_PLAYING){
				...
				warpLogger.log("next song: " + musicQueue[currentQueuePosition]);
				if(!loopMusicQueue){
