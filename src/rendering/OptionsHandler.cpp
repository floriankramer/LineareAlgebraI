...
namespace warp{
	namespace options{
		...
		void saveOptions(){
			...
			if(out.is_open()){
				char buffer[9];
				buffer[0] = options::isFullscreen();

				buffer[1] = options::getFBOWidth();
				buffer[2] = options::getFBOWidth() >> 8;

				buffer[3] = options::getFBOHeight();
				buffer[4] = options::getFBOHeight() >> 8;
		...
		void loadOptions(){
			...
			if(in.is_open()){
				...
				setFullscreen(buffer[0]);
				options::setResolution(((buffer[2] << 8) & 0xFF00) + (buffer[1] & 0xFF), ((buffer[4] << 8) & 0xFF00) + (buffer[3] & 0xFF));
