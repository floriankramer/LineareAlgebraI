...
bool PNG::load(){
	...
	if(!f){
		warpLogger.log(s + src.c_str());
		return false;
	}

	fread(header, 1, 8, f);
	if(png_sig_cmp(header, 0, 8)){
		warpLogger.err("the file %s is not a png file", src.c_str());
		return false;
	}

	png_structp pngStructp = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL);
	if(!pngStructp){
		warpLogger.err("png_create_read_struct failed for: %s", src.c_str());
		return false;
	}
	...
	if(!pngInfop){
		warpLogger.err("png_create_info_struct failed for: %s", src.c_str());
		return false;
	}
	...
	if(setjmp(png_jmpbuf(pngStructp))){
		warpLogger.err("initializing init_io failed for png file: %s", src.c_str());
		return false;
	}
	...
	if(setjmp(png_jmpbuf(pngStructp))){
		warpLogger.err("error trying to read png: %s", src.c_str());
		return false;
	}
